...
class MKF():
    def __init__(self, param_mtx, P, lamda, miu_v, miu_p, sig_r=0.01, sig_q=0.01):
    ...
    def adapt(self, x, err):
        H = np.copy(x).transpose() # 1xn
        tmp = np.linalg.inv(np.matmul(np.matmul(H, self.P), np.transpose(H)) + self.sig_r * np.identity(1))
        K = np.matmul(np.matmul(self.P, np.transpose(H)), tmp) # Kalman gain
        if(self.first):
            self.V = np.matmul(K, np.transpose(err))
            self.first = 0
        else:
            self.V = self.miu_v*self.V + (1-self.miu_v) * np.matmul(K, np.transpose(err))
